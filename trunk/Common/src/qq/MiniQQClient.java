...
public class MiniQQClient {
	...
	private String qq = "2586686564";
	private String password = "kk041kkstar";
	...
	public static void main(String[] args) {
		MiniQQClient clinet = new MiniQQClient("2644760340", "kk041kkstar");
		try {
	...
	private boolean login() {
		...
		String loginUrl = "http://ptlogin2.qq.com/login?u="
				...
				+ "&"
				+ "p="
				+ encodePass(this.password, check)
				+ "&verifycode="
				+ check
		...
		try {
			...
			if (retJson.getInt("retcode") == 0) {
				...
				for (ListIterator<JSONObject> it = infos.listIterator(); it
					...
					User user = new User(obj.getString("uin"),
							obj.getString("nick"), obj.getInt("face"),
							obj.getLong("flag"));
	...
	public boolean sendMsg(String toQQ, String message, String toGroup) {
		try {
	...
	public void receiveMsg(JSONObject value, boolean isGroup) throws Exception {
		if (!isGroup) {
			...
			messageProcessor.processMessage(group_uin, content, reply_ip);
			Thread.sleep(500);
		}
	}
	...
	class PollMessageThread extends Thread {
		...
		public void run() {
			...
			while (run) {
				try {
					String ret = sendHttpMessage(pollUrl, METHOD.GET.name(),
							null);
